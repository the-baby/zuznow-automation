...
.then( _ => z.clickById('bigNewFaqBtn'))
...
//.then( _ => z.clickById('faq_advanced_config'))
...
//.then( _ => z.clickByCss('#faq_gen_container button.close'), 'the close button')
...
driver
.then( _ => z.scenario('It is possible to select all intents and import them into a newly created folder'))
.then( _ => driver.switchTo().defaultContent())
.then( _ => z.clickById('bigNewFaqBtn'))
.then( _ => z.clickByCss('#faq_gen_header_selectAll>input'))
.then( _ => z.clickById('faqFolderSelect'))
.then( _ => z.clickByCss('#faqFolderSelect > option:nth-child(1)'))
.then( _ => z.inputById('faq_new_folder', 'Test'))
.then( _ => z.waitFor(3))
.then( _ => z.clickById('faq_gen_import'))
.then( _ => z.waitFor(10))
.then( _ => z.clickById('bigNewFaqBtn'))
...
.then( _ => z.clickByCss('#faq_gen_container button.close'), 'the close button')
.then( _ => z.assertExistsByName('Test'), 'the Test folder was added')
.catch( z.failedScenario )
/*
scenario('Clicking Reset button removes all the info')
.then( _ => z.clickById('faq_gen_back'))
.then( _ => z.clickById('faq_gen_reset'))
.then( () => z.assertContainsEmptyValue(By.id('faq_url'), "value should be empty") )
.then( _ => z.clickByCss('#faq_gen_container button.close'), 'the close button')
.catch( z.failedScenario )
*/
