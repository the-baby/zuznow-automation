...
.then( _ => z.waitFor(6))
.then( () => z.assertContainsText(By.id('conv-wrap'), "the expected text in the element", 'at +1800-000-000') )
.catch( z.failedScenario )
...
.then( _ => z.clickByCss('.panel[name="Contact"]  .action-link[action-name="PhoneCall"]'))
.then( _ => z.waitFor(4))
//.then( _ => z.clickByCss('.modal-content .textRes_tr:not(.template):last-child  .fa-trash-o')) check
//.then( _ => z.waitFor(4))
//.then( _ => z.clickById ('customModalButton'))
.then( _ => z.waitFor(2))
.then( _ => z.changeInputByCss('.modal-content input.param_value.form-control[name="phoneNumber"]', "+1 (646) 869-2931"))
...
.then( _ => z.clickByClassName('submitBtn form-control c1Icon c1Icon-paper-plane'))
.then( _ => z.waitFor(4))
.then( () => z.assertContainsText(By.id('conv-wrap'), "the expected text in the element", '+1 (646) 869-2931.') )
.catch( z.failedScenario )
...
.then( _ => z.clickByClassName('submitBtn form-control c1Icon c1Icon-paper-plane'))
.then( _ => z.waitFor(6))
.then( () => z.assertContainsText(By.id('conv-wrap'), "the expected text in the element", 'Did you mean "Please contact support"?') )
.catch( z.failedScenario )
