...
.then( _ => driver.switchTo().frame("chatbot-preview"))
.then( _ => z.waitFor(6))
.then( _ => z.inputById('input','Are you a bot'))
.then( _ => z.waitFor(4))
.then( _ => z.clickByClassName('submitBtn form-control c1Icon c1Icon-paper-plane'))
.then( _ => z.waitFor(4))
.then( () => z.assertContainsText(By.id('conv-wrap'), "the expected text in the element", 'Everyone has their goods and bads...') )
.catch( z.failedScenario )
...
.then( _ => z.inputByClassName('textRes_textarea form-control', 'res.say("In this shop");')) 
.then( _ => z.clickByClassName('btn  button-next'))
.then( _ => z.waitFor(12))
.then( _ => driver.switchTo().frame("chatbot-preview"))
.then( _ => z.waitFor(6))
...
.then( _ => z.clickByClassName('submitBtn form-control c1Icon c1Icon-paper-plane'))
.then( _ => z.waitFor(4))
.then( () => z.assertContainsText(By.id('conv-wrap'), "the expected text in the element", 'In this shop') )                  
